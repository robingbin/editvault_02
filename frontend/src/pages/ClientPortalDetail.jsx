...
export default function ClientPortalDetail() {
  ...
  const forReview = items.filter((v) => ['Sent To Client', 'Done'].includes(v.editor_status) && !['Approved', 'Posted'].includes(v.client_status));
  ...
  const setStatus = (v, s) => { v.client_status = s; refresh(); toast.success(`Marked ${s}`); };
              ...
              <div key={v.id} className="rounded-xl border border-[#152223] bg-[#0a1112] overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => setStatus(v, 'Approved')} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-[#1e5a3d] bg-[#0e2a1e] text-[#4ade80] text-xs font-medium hover:bg-[#123b2b]"><CheckCircle2 className="w-3.5 h-3.5" /> Approve</button>
      ...
      <Dialog open={correctionOpen} onOpenChange={setCorrectionOpen}>
        <DialogContent className="bg-[#0a1112] border border-[#243334] text-[#e6f7f6]">
          ...
          <DialogFooter>
            ...
            <button onClick={() => { if (target) { setStatus(target, 'Correction'); setCorrectionOpen(false); setNote(''); setTarget(null); } }} className="px-3.5 py-2 rounded-lg bg-[#2dd4bf] hover:bg-[#26c1ad] text-[#062626] text-sm font-semibold">Submit</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
